...
const audienceTypes = ["Creators", "Businesses", "Influencers", "Bloggers", "Agencies"];
      ...
      <div className="mx-auto flex max-w-7xl items-center justify-between px-4 py-4 sm:px-6 lg:px-8">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs font-medium text-slate-500">Affiliate partner directory</p>
          </div>
        </div>
        ...
        <div className="hidden items-center gap-3 md:flex">
          <button className="rounded-xl px-4 py-2 text-sm font-semibold text-slate-700 hover:bg-slate-100">Sign in</button>
          <button className="rounded-xl bg-slate-950 px-4 py-2 text-sm font-semibold text-white shadow-sm hover:bg-slate-800">List your program</button>
        </div>
      ...
      {open && (
        <div className="border-t bg-white px-4 py-4 md:hidden">
          <div className="grid gap-3 text-sm font-semibold text-slate-700">
            ...
            <button className="mt-2 rounded-xl bg-slate-950 px-4 py-3 text-white">List your program</button>
          </div>
        </div>
      )}
    </header>
              ...
              <div key={program.name} className="rounded-3xl border bg-slate-50 p-4">
                <div className="flex items-start justify-between gap-4">
                  <div>
                    ...
                    <p className="mt-1 text-sm text-slate-600">{program.audience}</p>
                  </div>
...
function Directory() {
  ...
  const filtered = useMemo(() => {
    return programs.filter((program) => {
      ...
      const queryMatch = [program.name, program.category, program.audience, ...program.tags]
        .join(" ")
        .toLowerCase()
        ...
        <div className="mt-8 grid gap-4 rounded-[2rem] border bg-slate-50 p-4 lg:grid-cols-[1fr_auto]">
          <div className="flex items-center gap-3 rounded-2xl bg-white px-4 py-3 shadow-sm">
            ...
            <input
              ...
              placeholder="Try: finance, beauty, pets, recurring, SaaS"
            />
          </div>
    ...
    <section id="brands" className="bg-slate-950 py-20 text-white">
      <div className="mx-auto grid max-w-7xl gap-10 px-4 sm:px-6 lg:grid-cols-2 lg:px-8 lg:items-center">
        <div>
          ...
          <p className="mt-4 text-slate-300">List your affiliate program, verify your offer, and reach creators, publishers, agencies, and businesses searching for partner opportunities in your niche.</p>
          <div className="mt-8 grid gap-4 sm:grid-cols-2">
            {["Verified program badge", "Creator lead requests", "Featured niche placement", "Performance analytics"].map((item) => (
...
function Pricing() {
  const tiers = [
    { name: "Creator", price: "Free", text: "Search programs, save favorites, and build a shortlist.", cta: "Start searching" },
    { name: "Pro Creator", price: "$19/mo", text: "Advanced filters, alerts, outreach templates, and hidden opportunities.", cta: "Upgrade" },
    { name: "Brand Listing", price: "$99/mo", text: "Verified listing, lead requests, niche placement, and analytics.", cta: "List program" },
  ];
  return (
        ...
        <div className="mt-10 grid gap-5 lg:grid-cols-3">
          {tiers.map((tier) => (
            <div key={tier.name} className="rounded-[2rem] border bg-slate-50 p-6">
              ...
              <button className="mt-6 w-full rounded-2xl bg-slate-950 px-5 py-3 font-black text-white hover:bg-slate-800">{tier.cta}</button>
            </div>
          ))}
        </div>
      </div>
  ...
  return (
    <footer className="border-t bg-slate-50">
      <div className="mx-auto grid max-w-7xl gap-8 px-4 py-10 sm:px-6 lg:grid-cols-[1.2fr_0.8fr_0.8fr_0.8fr] lg:px-8">
        <div>
          <div className="flex items-center gap-3">
            <div className="flex h-10 w-10 items-center justify-center rounded-2xl bg-slate-950 text-white"><BadgeDollarSign className="h-6 w-6" /></div>
            <p className="text-xl font-black text-slate-950">PartnerScout</p>
          </div>
          <p className="mt-4 max-w-sm text-sm leading-6 text-slate-600">The sourcing directory for affiliate programs, partner opportunities, creator monetization, and brand discovery.</p>
        </div>
        <div>
        ...
        <div>
          ...
          <div className="mt-4 grid gap-3 text-sm text-slate-600"><a>List program</a><a>Verify listing</a><a>Advertise</a></div>
        </div>
        <div>
          <p className="font-black text-slate-950">Contact</p>
          <div className="mt-4 grid gap-3 text-sm text-slate-600"><a className="flex items-center gap-2"><Mail className="h-4 w-4" /> hello@partnerscout.com</a><a className="flex items-center gap-2"><ExternalLink className="h-4 w-4" /> Partner resources</a></div>
        </div>
      </div>
    </footer>
  );
